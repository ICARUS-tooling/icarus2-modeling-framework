 ...
 *
 */
class FragmentLayerManifestXmlDelegateTest implements
		ManifestXmlDelegateTest<FragmentLayerManifest, FragmentLayerManifestXmlDelegate> {

	/**
	 * @see de.ims.icarus2.test.GenericTest#getTestTargetClass()
	 */
	@Override
	public Class<? extends FragmentLayerManifestXmlDelegate> getTestTargetClass() {
		return FragmentLayerManifestXmlDelegate.class;
	}
	...
	public void testMandatoryConstructors() throws Exception {
		...
		FragmentLayerManifest layerManifest = mockManifest();
		assertEquals(layerManifest, new FragmentLayerManifestXmlDelegate(layerManifest).getInstance());

		LayerGroupManifest groupManifest = mockTypedManifest(ManifestType.LAYER_GROUP_MANIFEST, true);
		assertOptionalEquals(groupManifest, new FragmentLayerManifestXmlDelegate(groupManifest).getInstance().getGroupManifest());
	}

	@Test
	void testResetLayerGroup() {
		FragmentLayerManifestXmlDelegate delegate = create();
		LayerGroupManifest groupManifest = mockTypedManifest(ManifestType.LAYER_GROUP_MANIFEST, true);
