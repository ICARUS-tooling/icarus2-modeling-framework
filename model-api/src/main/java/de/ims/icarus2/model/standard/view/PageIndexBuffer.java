...
public class PageIndexBuffer {
	...
	public PageIndexBuffer(IndexSet[] indices, int pageSize) {
		...
		if(indices.length==0)
			throw new ModelException(ModelErrorCode.INVALID_INPUT, "Indices array is empty");
		if(pageSize<=0)
			throw new ModelException(ModelErrorCode.INVALID_INPUT, "Page size must not be zero or negative: "+pageSize);
