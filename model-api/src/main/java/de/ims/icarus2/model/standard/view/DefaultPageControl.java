 ...
 * $URL: https://subversion.assembla.com/svn/icarusplatform/trunk/Icarus2Core/core/de.ims.icarus2.model/source/de/ims/icarus2/model/standard/view/DefaultPageControl.java $
 *
 ...
 * @version $Id: DefaultPageControl.java 457 2016-04-20 13:08:11Z mcgaerty $
 *
 */
public class DefaultPageControl extends AbstractPart<CorpusView> implements PageControl {
	...
	public boolean loadPage(int index) throws InterruptedException {
		...
		try {
			...
			if(indices==null)
				throw new ModelException(ModelErrorCode.MISSING_DATA, "No valid IndexSet available for page: "+pageIndex);
	...
	protected void closePage0() throws InterruptedException {
		...
		try {
			...
			throw new ModelException(ModelErrorCode.DELEGATION_FAILED, "Failed to close page: "+pageIndex, e);
		} finally {
	...
	protected void loadPage0(final int pageIndex, final IndexSet indices) throws InterruptedException {
		...
		try {
			...
			firePageFailed(pageIndex, e);
			currentPage.setPageState(PageState.BLANK);
			throw new ModelException(ModelErrorCode.DELEGATION_FAILED, "Failed to load page: "+pageIndex, e);
		}
	...
	protected static class PageLock {
		...
		public synchronized void unlock(Object key) {
			...
			if(currentKey==key) {
				...
				throw new ModelException(ModelErrorCode.INVALID_INPUT, "Page currently not locked by key: "+key);
		}
