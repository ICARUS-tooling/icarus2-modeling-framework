...
Note that legal arguments for a node that occurs in a container of size $N$ reside in the closed interval $[1..N]$ for regular index values and $[-N..-1]$ for reverse indices (see below).
...
\begin{tabular}{|p{0.17\textwidth}|p{0.14\textwidth}|p{0.60\textwidth}|}
	...
	\hline 
	isLast & - & Node is the last in its context ($index = N-1$). \\  
	\hline 
	isAfter & int & Node occurs after designated position ($index > arg$). \\  
	\hline 
	isBefore & int & Node occurs before designated position ($index < arg$). \\  
	\hline 
...
\paragraph{Negation}
\noindent Note that position markers cannot be negated when used in a constraint expression.\footnote{Depending on the query configuration doing so would either fail the evaluation process with an error or silently ignore the issue, potentially producing invalid results.}
...
\noindent Position markers may \textbf{only} be used as top-level terms in a constraint expression.
\iql allows multiple position markers to occur in the same constraint expression, but only if they are all joined by a conjunctive Boolean connective!
\begin{Verbatim}
// legal use cases
[isLast]
[pos!="NN" && isBefore(10)]

// illegal use of position markers
[pos!="NN" || isLast]
\end{Verbatim}

\paragraph{Markers in Global Constraints}
\noindent Global constraints (\ref{sec:global-constraints}) have access to an extended version of the ``local'' position markers.
\todo{explain extra first argument (the desired item to be checked), apart from that they behave the same}

\subsection{Tree Functions}
\label{sec:tree-functions}

\todo{describe the situations possible for nodes (root, top-level, nested) with implicit/explicit parent, and then list the isFirstChild, isLastChild, etc markers}
