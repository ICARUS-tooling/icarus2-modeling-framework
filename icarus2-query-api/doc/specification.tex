...
\noindent Note that those evaluations are only active if the respective switches (\ref{sec:json-ld-switches}) have not been set.
...
\begin{grammar}	
	...
	<selection> ::= (`WITH' <bindingsList>)? `FIND' <modifier>? <constraints>
...
Constraints can either be predicates, loop predicates, bracketed (with \keyword{(} or \keyword{)}) constraints or Boolean disjunctions (via \keyword{OR}) or conjunctions (with \keyword{AND}) of constraints.
...
They consist of arbitrary basic constraints \cref{sec:basic-constraints} and typically make global constraints redundant.
...
That is, in a node sequence \verb|[$x:][$y:]| the node matched by \texttt{x} will \textbf{always} be before the node matched by \texttt{y} according to the original direction of items in the corpus.
...
\noindent Empty nodes with quantifiers can be used as proxies to model distance constraints, as seen in above examples.
...
\noindent Consider a simple tree query \verb|[$x: [$y:]]| where x and y are bound nodes with individual constraints.
The nodes X\textsubscript{n} or Y\textsubscript{n} in the following example tree then denote the n-th node that match the constraints of query nodes x and y, respectively:
...
\noindent The tree contains 3 possible matches for \verb|[$x: [$y:]]|, specifically the pairs \texttt{\{X\textsubscript{1},Y\textsubscript{1}\}}, \texttt{\{X\textsubscript{1},Y\textsubscript{2}\}} and \texttt{\{X\textsubscript{2},Y\textsubscript{2}\}}.
...
Global constraints con be any basic constraint \cref{sec:basic-constraints} and follow after the main section of structural constraint, indicated yb the \keyword{HAVING} keyword.
