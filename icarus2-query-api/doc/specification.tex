...
\begin{tabular}{|p{0.2\textwidth}|p{0.42\textwidth}|p{0.17\textwidth}|}
	\hline
...
\begin{description}
	...
	\item[\iqlType{name}] The identifier used by the query engine's manifest registry for the corpus. %TODO link to rgeistry class
\end{description}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{description}
	...
	\item[\iqlType{quantifiers}] Optional quantifiers to define the multiplicity of matches of this node required for a positive evaluation.
\end{description}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{elements}{\iqlType{TreeNode}}
	\element{children}{array of \iqlBaseType{Element} (\ref{sec:json-ld-element})}{no}
\end{elements}
\begin{description}
	\item[\iqlType{children}] Optional list of nested nodes or node alternatives.
\end{description}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%\item resultType
%\end{itemize}
\desc{Wraps the textual form of an arbitrarily complex \iql expression, which can be formula, literal, method invocation, a combination of those or a great many other types of expressions. FOr more details see \cref{sec:expressions}.}
...
\begin{elements}{\iqlType{XXX}}
	\element{xxx}{array of \iqlType{XXX} (\ref{sec:json-ld-xxx})}{no}
\end{elements}
\begin{description}
	\item[\iqlType{xxx}] XXX.
\end{description}
...
\end{attributes}
\begin{description}
	\item[\iqlType{id}] Identifier to uniquely identify the payload within the entire query.
	\item[\iqlType{name}] Identifier to uniquely identify the payload within the entire query.
	\item[\iqlType{xxx}] XXX.
\end{description}
\begin{elements}{\iqlType{XXX}}
	\element{xxx}{array of \iqlType{XXX} (\ref{sec:json-ld-xxx})}{no}
...
\end{elements}
\begin{description}
	\item[\iqlType{layers}] The members of this scope.
\end{description}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\end{attributes}
\begin{description}
	\item[\iqlType{order}] Hint on sorting direction, legal values are \texttt{asc} or \texttt{desc}.
\end{description}
...
%TODO add unicode escape mechanism
...
\noindent Identifiers are limited in length by the engine to a total of 255 characters. 
...
\noindent\begin{tabular}{|l|c|c|c|c|c|c|}
	\hline 
Type	& Prefix & Example & Scope & Fixed\footnote{Fixed as in ``fixed type'', i.e. the type of value stored is pre-determined and cannot be changed during query evaluation.} & Final & Re-Assign \\ 
	\hline 
...
\noindent The first expression must evaluate to a \keyword{boolean} value and determines which of the following two alternatives will be evaluated for the total value of the expression. Note that the second and third expressions must have compatible result types.
