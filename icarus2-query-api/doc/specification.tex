...
\begin{gram}[\synt{set_predicate}]
	...
	\begin{rrdiag*}{set_predicate}
		<expression> \sst \\ `ALL' \\ `*' \est \sst \\ `NOT' \\ `!' \est `IN' `{' \srp <expression> \\ `,' \erp `}'
	\end{rrdiag*}
\end{gram}
...
\todo{add syntax diagram}

\subsubsection{Predicate}
\label{sec:predicate}

Predicates are essentially expressions (\ref{sec:expressions}) that evaluate to a Boolean value. 
See \cref{tab:constraints-translation} for information on how non-Boolean types are interpreted as Boolean values and the switches (\ref{sec:json-ld-switches}) section for ways to influence this behavior.

\subsubsection{Loop Predicate}
...
They consist of arbitrary basic constraints \cref{sec:basic-constraints} and disallow both global constraints (\ref{sec:global-constraints}) and filter constraints (\ref{sec:filter-constraints}). \todo{a word on the differences between flat and global constraints}
...
Local constraints have full access to properties and annotations of the target item currently being inspected and can take any form described in \cref{sec:basic-constraints}.
...
Correct evaluation of a conjunctive local constraints with external function calls \verb|[$x: func1($x) && func2($x) && func3($x)]| must not rely on the premise of any particular function (\texttt{func1}, \texttt{func2} or \texttt{func3}) actually being called at all.
...
\noindent Nodes (or elements) can be grouped together within curly brackets (\keyword{\{}~and~\keyword{\}}).
...
\noindent Nodes usable for sequences (\ref{sec:sequence-constraints}) and trees (\ref{sec:tree-constraints}) are defined in a simple sequence style.
