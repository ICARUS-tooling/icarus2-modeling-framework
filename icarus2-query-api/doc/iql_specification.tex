...
\end{attributes}
\begin{description}
	\item[\iqlType{arrangement}] Defines what kind of order or arrangement should be assumed between the elements in this sequence. Legal values are \texttt{unordered}, \texttt{ordered} (matched elements must occur in exactly the order specified in this set but need not form a continuous span) or \texttt{adjacent} (matched elements must form a continuous span).
\end{description}
...
\begin{gram}[\synt{payload}]
	...
	\begin{grammar}	
		<payload> ::= `ALL' 
		\alt <bindingsList>? (`FILTER' `BY' <constraint>)? `FIND' <hitsLimit>? <flag>* <selection>
		
		<hitsLimit> ::= <digits> `HITS'
	...
	\begin{rrfrag}{...}{->}
		`FIND'
			\sst \\ <digits> `HITS' \est
			\sst \\ `DISJOINT' \\ `CONSECUTIVE' \\ `ROOTED' \\ `REVERSE' \est <selection>
	\end{rrfrag}
	\end{samepage}
\end{gram}
...
\subsection{Hits Limit}
\label{sec:hits-limit}
...
This default behavior can be adjusted to return each \ac{uoi} only up to a certain time, via the \keyword{HITS} keyword with a positive non-zero integer in front that follows the syntax of ``pure integers'' as described in \cref{sec:integer-literals}.

In case that only a single match needs to be reported, the \keyword{FIRST} keyword will achieve the same result as \query{1 HITS}. The latter

\subsection{Match Flags}
\label{sec:match-flags}

In addition to the hits limit (\cref{sec:hits-limit}) a series of flags can also be used to influence the matching process.
Those flags are acting as binary switches to activate certain features.
Currently the following match flags are available, with mutually exclusive flags marked by an asterisk (*):
\paragraph{\keyword{CONSECUTIVE} *}
...
For queries that enforce continuous groups of nodes to be matched in a target sequence, this will produce the same results as the \keyword{DISJOINT} flag.
If non-continuous target sequences can be matched, this flag may skip over content that could produce valid hits.
\paragraph{\keyword{DISJOINT} *}
In contrast to the \keyword{CONSECUTIVE} flag, this one uses set semantics as basis.
...
\paragraph{\keyword{ROOTED} *}
Per default the search engine uses the natural order of elements in a target structure as basis for the top-level scan. 
When the first node in the structural constraint is a \iqlType{TreeNode} (\cref{sec:json-ld-tree-node}) the special \keyword{ROOTED} flag can be used.
This will turn the scan to be performed on all the root nodes instead, turning it into a pure top-down search instead.
\paragraph{\keyword{REVERSE}}
This flag simply causes the top-level scan to be performed back to front in relation to the natural order of elements in the target structure.
Note however that this does \textbf{not} affect the direction of nested scans!
...
To illustrate the interaction of quantifiers and markers, consider the example of a node \query{<3..5>[\$x: isInside(0,0.25),]}.
