...
import de.ims.icarus2.model.standard.driver.file.FileMetadata.MappingKey;
...
public abstract class FileDriver extends AbstractDriver {
	...
	private static Logger logger = LoggerFactory.getLogger(FileDriver.class);
	...
	protected Mapping defaultCreateMapping(MappingFactory mappingFactory, MappingManifest mappingManifest, Options options) {
		...
		ContextManifest contextManifest = mappingFactory.getDriver().getManifest().getContextManifest();
		...
		return super.defaultCreateMapping(mappingFactory, mappingManifest, options);
	}
			...
			getMappings().forEachMapping(m -> {
				...
				if(AbstractStoredMapping.class.isInstance(m)) {
					...
					try {
						...
						logger.error("Failed to delete mapping storage", e);
					}
				}
			});
		} finally {
	...
	protected void doConnect() throws InterruptedException {
		...
		for(int i=0; i<steps.length; i++) {
			...
			try {
				...
				logger.error("Preparation step {} of {} failed: {}", i+1, steps.length, step.name(), e);
				valid = false;
			}
	...
	public static class FileDriverBuilder extends DriverBuilder<FileDriverBuilder, FileDriver> {
		...
		protected void validate() {
			...
			checkState("Missing file connector", dataFiles!=null);
